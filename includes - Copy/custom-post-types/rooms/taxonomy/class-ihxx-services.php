...
if ( ! class_exists( '\LoftOcean\Room\ihxx_services' ) ) {
    class ihxx_services {
        /**
        * String Post type
        */
        ...
        * Taxomony type
        */
        public $taxonomy = 'lo_room_ihxx_services';
        /**
        * Construct function
        */
        public function __construct() {
            ...
            add_filter( 'loftocean_get_room_ihxx_services', array( $this, 'get_ihxx_services' ) );
            add_filter( 'loftocean_get_room_ihxx_services_enabled', array( $this, 'get_room_enabled_ihxx_services' ), 10, 2 );
            add_filter( 'loftocean_get_room_detailed_ihxx_services', array( $this, 'get_room_enabled_detailed_services' ), 10, 2 );
        }
        ...
        */
        public function custom_posttype() {
            register_taxonomy( $this->taxonomy ,array( $this->post_type ), array(
                    'hierarchical' => false,
                    'labels' => array(
                    'name' => esc_html__( 'نوع اتاق', 'loftocean' ),
                    'singular_name' => esc_html__( 'نوع اتاق', 'loftocean' )
                ),
                'show_ui' => false,
                'public' => false,
                'show_in_rest' => false,
                ...
                'query_var' => true,
                'rewrite' => array( 'slug' => 'room-ihxx-service' ),
                'meta_box_cb' => false
            ) );
        }
        ...
        */
        public function add_admin_menu() {
            $label = esc_html__( 'نوع اتاق', 'loftocean' );
            add_submenu_page( 'edit.php?post_type=' . $this->post_type, $label, $label, 'manage_options', 'loftocean_room_ihxx_services', array( $this, 'room_ihxx_services_settings_page' ) );
        }
        /*
        * Room ihxx services setting page
        */
        public function room_ihxx_services_settings_page() {
            $this->save_room_ihxx_services();

            wp_enqueue_script( 'admin-room-ihxx-service', LOFTOCEAN_URI . 'assets/scripts/admin/room-ihxx-service.min.js', array( 'jquery', 'wp-util', 'jquery-ui-sortable', 'jquery-ui-datepicker' ), LOFTOCEAN_ASSETS_VERSION, true );
            wp_localize_script( 'admin-room-ihxx-service', 'loftoceanRoomihxxServices', $this->get_ihxx_services() );
            wp_enqueue_style( 'admin-room-ihxx-service', LOFTOCEAN_URI . 'assets/styles/room-ihxx-service.min.css', array(), LOFTOCEAN_ASSETS_VERSION );
            wp_enqueue_style( 'jquery-ui', '//code.jquery.com/ui/1.13.1/themes/base/jquery-ui.css', array(), '1.13.1' );

            require_once LOFTOCEAN_DIR . 'includes/custom-post-types/rooms/view/taxonomy/page-ihxx-service.php';
        }
        /**
        * Save room facilitie settings
        */
        protected function save_room_ihxx_services() {
            if ( current_user_can( 'manage_options' ) && isset( $_REQUEST['loftocean_room_ihxx_services_settings_nonce'] )
                && wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['loftocean_room_ihxx_services_settings_nonce'] ) ), 'loftocean_room_ihxx_services_settings_nonce' ) ) {
                $ihxx_services = isset( $_REQUEST[ 'loftocean_room_ihxx_service' ] ) ? wp_unslash( $_REQUEST[ 'loftocean_room_ihxx_service' ] ) : false;
                if ( \LoftOcean\is_valid_array( $ihxx_services ) ) {
                    $ihxx_services = array_filter( $ihxx_services, function( $item ) {
                        return ! empty( $item[ 'title' ] );
                    } );
                    if ( \LoftOcean\is_valid_array( $ihxx_services ) ) {
                        $priority = 0;
                        foreach ( $ihxx_services as $es ) {
                            $term_id = $es[ 'id' ];
                ...
                $removed_terms = empty( $_REQUEST[ 'loftocean_room_ihxx_service_removed' ] ) ? false : sanitize_text_field( wp_unslash( $_REQUEST[ 'loftocean_room_ihxx_service_removed' ] ) );
                if ( $removed_terms ) {
        ...
        */
        public function get_ihxx_services( $ihxx = array() ) {
        ...
        /**
        * Get enabled ihxx services of a given room ID
        */
        public function get_room_enabled_ihxx_services( $services, $room_id ) {
        ...
        /**
        * Get detailed enabled ihxx service for given room
        */
